...
Reads tour metadata from tour_data.json, hydrates the stop content from the
live tour JSON sources, and produces BSH_2026_Tour_Content.docx for editorial review.
"""
            ...
            for section in stop.get("sections", []):
                ...
                if section_type == "highlights":
                    ...
                    for image in section.get("images", []):
                        caption = image.get("caption") or image.get("alt") or image.get("src", "")
                        parts.append(rtf_paragraph([rtf_run(f"• {caption}")], indent=180, first_indent=-180, space_after=30))
...
def load_tours(script_dir):
    """Load tour metadata and hydrate stops from the live map JSON sources."""
    json_path = os.path.join(script_dir, "tour_data.json")
    with open(json_path, "r", encoding="utf-8") as f:
        tours = json.load(f)

    live_sources = [
        os.path.join(script_dir, "fscclt", "data.json"),
        os.path.join(script_dir, "community-orgs", "data.json"),
    ]

    for tour, source_path in zip(tours, live_sources):
        with open(source_path, "r", encoding="utf-8") as f:
            tour["stops"] = json.load(f)
